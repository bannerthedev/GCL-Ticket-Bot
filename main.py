...
TOKEN = "MTUwMTc4OTU1NjY5MTgzMjk2Ng.G1OUG8.VY8mZIAD_1mvlQ2mYTJTQvC2I1TtKCbXWwrw-s"
...
def menu_embed():
    ...
    e.description = (
        ...
        "**Support**\nUse this for general help, questions, or issues related to the league, server, or events.\n\n"
        "**Report Server Players**\nUse this to report a player for breaking server rules (toxicity, spamming, harassment, etc.). Provide proof when possible.\n\n"
        "**Report In-Game Player**\nUse this to report a player for in-game violations (cheating, exploiting, unfair play, etc.). Clips or screenshots are highly recommended.."
    )
    return e
...
class TicketMenuView(discord.ui.View):
    ...
    async def support(self, interaction: discord.Interaction, button: discord.ui.Button):
        await create_ticket_channel(interaction, "🟢 Supportt", self.category_id)

    @discord.ui.button(label="🟡 Report Server Player", style=discord.ButtonStyle.danger, custom_id="ticket_appeal")
    async def appeal(self, interaction: discord.Interaction, button: discord.ui.Button):
        await create_ticket_channel(interaction, "🟡 Report Server Player", self.category_id)

    @discord.ui.button(label="🔵 Report In-Game Player", style=discord.ButtonStyle.secondary, custom_id="ticket_management")
    async def management(self, interaction: discord.Interaction, button: discord.ui.Button):
        await create_ticket_channel(interaction, "🔵 Report in-game Player", self.category_id)
